    ...
    <div className={`bg-white rounded-xl shadow-md p-4 mb-3 border-l-4 ${
      ...
      <div className="flex justify-between items-start mb-2">
        <div className="flex items-center gap-2">
          <Clock className={`h-4 w-4 ${isExpired ? 'text-gray-400' : 'text-blue-500'}`} />
          <h3 className={`text-base font-semibold ${isExpired ? 'text-gray-500' : 'text-gray-800'}`}>
            {item.title}
          </h3>
          <span className={`text-xs px-2 py-1 rounded-full ${priorityTextColors[item.priority] || 'text-blue-500'} bg-gray-100`}>
      ...
      {isExpired ? (
        <div className="text-gray-500 font-medium">已过期</div>
      ) : (
